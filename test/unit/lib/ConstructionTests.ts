...
import { Construction } from '../../../lib/Constructions/Construction';
...
suite('Construction', (): void => {
	test('Add a hub to the construction.', async (): Promise<void> => {
		...
		assert
			.that(construction.getHubs().length)
			.is.equalTo(construction.getHubLength());
	...
	test('Switch hub left and check if first selected and step left last is selcted.', async (): Promise<void> => {
		...
		assert
			.that(construction.getCurrentHubIndex())
			.is.equalTo(construction.getHubLength() - 1);
